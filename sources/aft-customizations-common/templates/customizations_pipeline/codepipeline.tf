...
resource "aws_codepipeline" "aft_s3_customizations_codepipeline" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        S3Bucket       = data.aws_ssm_parameter.aft_global_customizations_s3_bucket_name.value
        S3ObjectKey           = data.aws_ssm_parameter.aft_global_customizations_bucket_object_key.value
        PollForSourceChanges = false
      }
    }
    ...
    action {
      ...
      configuration = {
        S3Bucket       = data.aws_ssm_parameter.aft_account_customizations_s3_bucket_name.value
        S3ObjectKey           = data.aws_ssm_parameter.aft_account_customizations_bucket_object_key.value
        PollForSourceChanges = false
      }
    }
  }
